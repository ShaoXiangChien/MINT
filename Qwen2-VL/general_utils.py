...
# 添加Qwen2-VL相關的常數和導入
IMAGE_TOKEN_INDEX = -200  # 需要根據Qwen2-VL實際情況調整


def prepare_inputs(prompt, image, processor, device):
   messages = [
    {
        "role": "user",
        "content": [
...
def tokenizer_image_token(text, tokenizer, image_token_index, return_tensors=None):
    """將文本中的圖像標記轉換為特殊標記"""
    # 針對Qwen2-VL進行調整
    ...
    if image_token in text:
        ...
        for chunk in text.split(image_token):
            ...
            text_chunks.append(image_token)
        text_chunks = text_chunks[:-1]  # 移除最後一個多餘的image_token
    ...
    if return_tensors == "pt":
        ...
        # Ensure the tensor is properly shaped (add batch dimension if needed)
