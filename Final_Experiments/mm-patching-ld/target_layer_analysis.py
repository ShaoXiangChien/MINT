#!/usr/bin/env python3
"""
Target Layer Analysis Script for MM-Patching Layer Intervention Experiments

This script creates a line chart showing the average injection accuracy per target layer
for vision-language models (QWEN2-VL, DeepSeek-VL2, and LLaVA-1.5) in mm-patching experiments.

Based on the text-patching analysis script but adapted for the mm-patching data structure.
"""
...
class MMPatchingAnalyzer:
    """Analyzer for mm-patching vision-language model intervention experiments."""
    ...
    def generate_target_layer_line_chart(self, save_path="./"):
        ...
        # Set integer ticks for x-axis (use the maximum range across all models)
        if self.processed_data:
            max_layers = max(len(info['layers']) for info in self.models.values() 
                           if info['layers'] is not None)
            ax1.set_xticks(range(0, max_layers, max(1, max_layers//10)))  # Show reasonable number of ticks
            ax2.set_xticks(range(0, max_layers, max(1, max_layers//10)))
    ...
    def print_summary_statistics(self):
        ...
        for model_name in self.models.keys():
            ...
            # Find best source-target pair
            best_idx = np.unravel_index(np.argmax(avg_matrix), avg_matrix.shape)
            best_source = best_idx[0]
            best_target_pair = best_idx[1]
            print(f"  Best source→target pair: Layer {best_source}→{best_target_pair} ({np.max(avg_matrix):.3f})")
    
    def run_analysis(self, save_path="./"):
        """Run the complete target layer analysis."""
        print("Starting mm-patching target layer analysis...")
        print("="*60)
...
def main():
    ...
    analyzer = MMPatchingAnalyzer(results_dir="./lg_results/")
