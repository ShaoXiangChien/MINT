#!/usr/bin/env python3
"""
Bootstrap Confidence Intervals for Text-Patching Experiments

This script mirrors the bootstrap CI pipeline used in global-img-ld but is
configured for the text-patching-ld results layout.

It computes nonparametric bootstrap confidence intervals for:
- Experimental accuracy (overall, per-matrix cell, per-target, per-source)
- Baseline accuracy (if provided or specified as a fixed constant)
- Intervention effect = Experimental − Baseline

Inputs (by default) are read from ./results/:
- DeepSeek-VL2 experimental: ds-results.json
- QWEN2-VL experimental:     qwen_results.json
- Optional baselines:         ds-baseline-results.json, qwen_baseline_results.json
  Baseline files should be a JSON list of dicts with key "prediction" (0/1).

Outputs are written to ./analysis_results/bootstrap_ci/:
- Per-model: {exp_filename_without_ext}_bootstrap.json
- Combined:  bootstrap_summary.json

Usage examples:
  python3 bootstrap_ci.py --iterations 10000 --confidence 0.95 \
    --baseline_strategy fixed --baseline_fixed 0.0

  # If you later add baseline files under results/, you can switch to:
  python3 bootstrap_ci.py --iterations 10000 --confidence 0.95 \
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Bootstrap CIs for text-patching experiments")
    ...
    parser.add_argument(
        "--baseline_strategy",
        type=str,
        choices=["file", "fixed", "none"],
        default="fixed",
        help="How to obtain baseline: from file, as a fixed constant, or none",
    )
    ...
    print("Bootstrapping confidence intervals (text-patching)...")
