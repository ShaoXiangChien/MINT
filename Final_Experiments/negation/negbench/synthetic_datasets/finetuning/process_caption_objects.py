"""
This script processes captions to extract relevant objects or generate related negative objects using the Meta-Llama-3.1-70B-Instruct model. 
The extracted objects or generated negative objects are saved to a new CSV file.

### Tasks:
1. **Extraction**: Extract objects or concepts explicitly mentioned in the captions.
2. **Negative**: Generate related objects absent from the captions but potentially associated with the extracted objects.

### Input CSV Requirements:
The input CSV file should contain the following columns:
- `caption`: Descriptive text of the image or video content.

### Output CSV:
The output CSV will include:
- `extracted_objects`: (if task_type is "extraction") Objects explicitly mentioned in the captions.
- `negative_objects`: (if task_type is "negative") Related objects absent from the captions.
...
python process_caption_objects.py \
    ...
    --output_base path/to/output_base_extracted \
    --task_type extraction \
    --index_start 0 \
    --index_end 100
```

#### Negative Object Generation:
```bash
python process_caption_objects.py \
...
#### Expected Output:
A CSV file with additional columns:
- For `--task_type extraction`: Adds a column `extracted_objects`.
- For `--task_type negative`: Adds a column `negative_objects`.

### Notes:
- Use `index_start` and `index_end` for chunk processing if working with large datasets.
- Ensure the `extracted_objects` column exists in the input file when running with `--task_type negative`.
- Ensure sufficient GPU resources are available for running the LLM model.
"""
...
5. Only provide the output with no additional text or explanation.
