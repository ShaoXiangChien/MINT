...
export PIP_CACHE_DIR="<CLUSTER_PIP_CACHE_DIR>"  # Placeholder: Replace with your cluster's pip cache directory path
...
CUDA_VISIBLE_DEVICES=0,1,2,3 python process_caption_objects.py --input_file $input_file --output_base $output_base --task_type negative --index_start $START_INDEX --index_end $END_INDEX > logs/negative_${FIRST_JOB_INDEX}.log 2>&1 &
...
CUDA_VISIBLE_DEVICES=4,5,6,7 python process_caption_objects.py --input_file $input_file --output_base $output_base --task_type negative --index_start $START_INDEX --index_end $END_INDEX > logs/negative_${SECOND_JOB_INDEX}.log 2>&1 &
