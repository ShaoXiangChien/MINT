...
CUDA_VISIBLE_DEVICES=0,1,2,3 python generate_negative_captions.py --model llama3.1 --input_file $input_file --output_base $output_base --index_start $START_INDEX --index_end $END_INDEX > logs/caption_${FIRST_JOB_INDEX}.log 2>&1 &
...
CUDA_VISIBLE_DEVICES=4,5,6,7 python generate_negative_captions.py --model llama3.1 --input_file $input_file --output_base $output_base --index_start $START_INDEX --index_end $END_INDEX > logs/caption_${SECOND_JOB_INDEX}.log 2>&1 &
