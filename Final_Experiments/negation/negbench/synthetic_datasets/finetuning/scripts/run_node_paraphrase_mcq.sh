...
input_file="${base_dir}/csvs/negation_dataset/combined/final/cc12m_mcq_captions.csv"
...
CUDA_VISIBLE_DEVICES=0,1,2,3 python paraphrase_captions.py --model llama3.1 --task mcq --input_file $input_file --index_start $START_INDEX --index_end $END_INDEX > logs/paraphrase_${FIRST_JOB_INDEX}.log 2>&1 &
...
CUDA_VISIBLE_DEVICES=4,5,6,7 python paraphrase_captions.py --model llama3.1 --task mcq --input_file $input_file --index_start $START_INDEX --index_end $END_INDEX > logs/paraphrase_${SECOND_JOB_INDEX}.log 2>&1 &
